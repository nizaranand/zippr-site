...
(function($){
	...
	var this_src = scripts[scripts.length-1].src;
	if(!(this_src.match("mColorPicker"))) {
		for(src in scripts) {
			if(src.match("mColorPicker.js")) {
				this_src = src;
				break;
			}
		}
	}
	var lastslash = this_src.lastIndexOf('/');
	this_src = this_src.substr(0, lastslash+1);
  ...
  $.fn.mColorPicker.drawPicker = function () {
    ...
    $(div).attr({
      ...
      'border': 0,
      'cursor': 'crosshair',
      'background-image': 'url(' + this_src + '../images/picker.png)'
    }).appendTo($w);
    ...
    $(img).attr({
      'src': this_src + '../images/meta100.png',
      'title': $i.slogan,
      'alt': $i.slogan
    }).css({
      'border': 0,
  ...
  $.fn.mColorPicker.colorTest = function (color) {
	...
	var out = $mColorPickerTest.css('background-color');
	
	// Webkit uses rgba(0, 0, 0, 0) instead of transparent
	if ((/^rgba\(0,\s*0,\s*0,\s*0\)$/i).test(out)) {
		out = "transparent";
	}
