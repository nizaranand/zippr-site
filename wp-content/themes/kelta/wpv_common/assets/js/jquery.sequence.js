		...
		goTo: function(id, direction){ //where the magic happens
			...
			if(id == self.currentFrame.index() + 1){
				...
				if(self.transitionsSupported){ //if the browser supports CSS3 transitions...
					...
					self.sequence.children("li").css({"position": "absolute"}); //this allows for fadein/out in IE
					self.currentFrame.siblings().children().addClass('animate-out');
					self.nextFrame.css({display: 'block'}).animate({opacity: 1}, 500);
					self.nextFrameChildren.removeClass('animate-out').addClass('animate-in');
					self.currentFrame.animate({"opacity": 0}, self.settings.fallbackTheme.speed, function(){ //hide the current frame
						...
						self.settings.beforeNextFrameAnimatesIn();
						self.nextFrame.addClass("current").css({"z-index": self.numberOfFrames}); //make the next frame the current one and show it
